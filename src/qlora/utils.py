...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    project_name: Optional[str] = field(default="debug")
    ...
    k: int = field(
        default=100, metadata={"help": "How much of the parameters should be pruned."}
    )
    replace_factor: float = field(
        default=-1,
        metadata={
            "help": "Replace_factor decides the constant value that is used for replacement."
        },
    )
    ...
    checkpoint_dir: str = field(
        default=None, metadata={"help": "Checkpoint to load the model from."}
    )
    optim: str = field(
...
def get_accelerate_model(args, checkpoint_dir):
    ...
    if not args.full_finetune:
        if checkpoint_dir is not None:
            ...
            compress_and_update_llama_adaptors(model, args.k, args.replace_factor)
        else:
